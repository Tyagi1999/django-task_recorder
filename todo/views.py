...
# superuser account - name:ashu, password:arrow1234
def home(request):
...
def loginuser(request):
    if request.method == 'GET':
        ...
        if user is None:
            return render(request, 'todo/loginuser.html', {'form':AuthenticationForm(), 'error':'Username and password not matched'})
        else:
            login(request, user)
            return redirect('currenttodos')
